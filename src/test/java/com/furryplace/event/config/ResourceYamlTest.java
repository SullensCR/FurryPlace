...
class ResourceYamlTest {
    ...
    void everyBundledMenuIsReadableAndHasValidInventoryGeometry() {
        for (String menu : MENUS) {
            ...
            assertEquals("1.0.3", yaml.getString("config-version"), menu);
    ...
    void primaryResourcesAreReadable() {
        assertEquals("1.0.3", load("config.yml").getString("config-version"));
        assertEquals("lobby", load("config.yml").getString("worlds.lobby"));
        assertEquals("1.0.3", load("messages.yml").getString("config-version"));
        ...
        assertEquals("1.0.3", load("biome-names.yml").getString("config-version"));
